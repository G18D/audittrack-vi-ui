...
/**
 * AuditTrack VI — Single-file React UI (no framer-motion)
 * Tailwind-only styling. Drop-in page for Next.js App Router.
 */
...
const Badge = ({
  ...
  <span
    ...
    style={{ background: `${palette[tone]}22`, color: palette[tone] }}
  >
    {children}
  </span>
);
    ...
    >
      <div className="flex items-center gap-2">
        <div
          ...
          style={{ background: `${palette[accent]}2A`, color: palette[accent] }}
        >
          {icon}
        </div>
...
const filesDemo = [
  {
    id: 1,
    name: "Invoice_000124.pdf",
    size: "412 KB",
    status: "Passed",
    issues: 0,
    vendor: "Carib Foods LLC",
  },
  {
    id: 2,
    name: "Payroll_May2025.csv",
    size: "38 KB",
    status: "Flagged",
    issues: 3,
    vendor: "HR Export",
  ...
  {
    id: 3,
    name: "Contract_UVI_Catering.pdf",
    size: "1.6 MB",
    status: "Manual Review",
    issues: 1,
    vendor: "Island Caterers",
  },
] as const;
...
const ProgressPill = ({ value }: { value: number }) => (
  <div className="h-3 w-full rounded-full bg-slate-200 overflow-hidden">
    <div
      ...
      style={{
        width: `${value}%`,
        background: `linear-gradient(90deg, ${palette.reef}, ${palette.lagoon})`,
      }}
    />
  </div>
);
  ...
  return (
    <aside className="h-full w-[280px] shrink-0 p-4">
      <div
        ...
        style={{
          background: `linear-gradient(180deg, ${palette.surf}, ${palette.sand})`,
          borderColor: `${palette.sea}22`,
        }}
      >
        <div className="flex items-center gap-3 mb-2">
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${palette.sea}, ${palette.reef})`,
            }}
          >
            AT
          </div>
          <div>
            <div className="text-sm tracking-wider font-semibold text-slate-800">
              AuditTrack{" "}
              <span
                className="text-[10px] align-top bg-white/70 px-1.5 py-0.5 rounded ml-1 border"
                style={{ borderColor: `${palette.sea}22` }}
              >
                VI
              </span>
            </div>
            <div className="text-[11px] text-slate-600">
              Design · Renovate · Build · Comply
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 flex flex-col gap-1">
          {items.map((it) => (
            <button
              ...
              style={{
                borderColor: `${palette.sea}18`,
                color: current === it.key ? palette.sea : "#0f172a",
              }}
            >
              {it.icon}
              {it.label}
        ...
        <div
          className="text-[11px] text-slate-600 pt-2 border-t"
          style={{ borderColor: `${palette.sea}22` }}
        >
          <div>Made for the U.S. Virgin Islands</div>
          <div className="mt-1">
            Accent <span style={{ color: palette.coral }}>●</span> coral red
          </div>
        </div>
      </div>
    </aside>
        ...
        <tbody>
          {filesDemo.map((f) => (
            <tr key={f.id} className="border-t last:border-b" style={{ borderColor: `${palette.sea}12` }}>
              ...
              <td className="py-3">
                <button className="text-xs underline hover:opacity-80" style={{ color: palette.sea }}>
                  View
                </button>
              </td>
            </tr>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2">
        <Card
          ...
          accent="sea"
          actions={<ActionButton icon={<Upload size={16} />} label="Upload PDFs" tone="sea" />}
        >
          <div className="text-sm text-slate-700">
            Drop IRS pubs, VI DOL guidance, GASB/OMB circulars, procurement rules. We'll OCR, chunk,
            embed (Pinecone), and index.
          </div>
          <div className="mt-4">
            ...
            <div className="mt-3 flex gap-2">
              <ActionButton
                ...
                onClick={() => setPct((p) => Math.min(100, p + 8))}
                tone="reef"
              />
      ...
      <div className="lg:col-span-1">
        <Card title="Sources" icon={<FileCheck2 size={16} />} accent="reef">
          ...
          <div className="mt-3 text-xs text-slate-500">
            Connect cloud buckets to auto-sync new revisions.
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2">
        <Card
          ...
          accent="reef"
          actions={<ActionButton icon={<Download size={16} />} label="Export CSV" tone="reef" />}
        >
          <FilesTable />
        </Card>
      </div>
      <div className="lg:col-span-1">
        <Card title="Auto-Fixes" icon={<ShieldCheck size={16} />} accent="mango">
          ...
          <div className="mt-3 text-xs text-slate-500">
            Manual approvals required for high-risk edits.
          </div>
        </Card>
      </div>
    </div>
      ...
      <Card title="Integrations" icon={<Settings size={16} />} accent="sea">
        <div className="grid grid-cols-2 gap-3 text-sm">
          <label className="flex flex-col gap-1">
            ...
            <input
              placeholder="audittrack-vi"
              className="rounded-xl border px-3 py-2"
              style={{ borderColor: `${palette.sea}33` }}
            />
          </label>
          <label className="flex flex-col gap-1">
      ...
      <Card title="Policies" icon={<ShieldCheck size={16} />} accent="mango">
        ...
        <div className="mt-3 grid grid-cols-3 gap-3">
          {["Lenient", "Standard", "Strict"].map((p) => (
            <button
              key={p}
              className="rounded-xl px-3 py-2 border text-sm font-semibold bg-white hover:shadow-sm"
              style={{ borderColor: `${palette.reef}33`, color: palette.sea }}
            >
              {p}
            </button>
          ))}
        ...
        <div className="mt-3 text-xs text-slate-500">
          Profiles control thresholds, auto-fix scope, dual-approval, and audit trail verbosity.
        </div>
      </Card>
    </div>
  );
...
export default function AuditTrackVIApp() {
  ...
  return (
    <div
      ...
      style={{ background: `linear-gradient(135deg, ${palette.surf}, ${palette.sand})` }}
    >
      {/* Top Bar */}
      <header
        className="sticky top-0 z-20 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b"
        style={{ borderColor: `${palette.sea}12` }}
      >
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          <div
            className="h-9 w-9 rounded-xl grid place-items-center text-white font-bold shadow"
            style={{ background: `linear-gradient(135deg, ${palette.coral}, ${palette.mango})` }}
          >
            VI
          </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            <input
              placeholder="Search invoices, rules, vendors…"
              className="rounded-xl px-3 py-2 text-sm border w-[280px]"
              style={{ borderColor: `${palette.sea}22` }}
            />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2">
                  <Card
                    ...
                    icon={<Upload size={16} />}
                    accent="reef"
                    actions={<ActionButton icon={<Trash2 size={16} />} label="Clear" tone="coral" />}
                  >
                    <UploadZone />
                  </Card>
                </div>
                  ...
                  <Card title="Compliance Health" icon={<ShieldCheck size={16} />} accent="mango">
                    ...
                    <div className="mt-2">
                      ...
                      <div className="mt-2 text-xs text-slate-600">
                        84/100 · Improving · Focus: payroll quarterly
                      </div>
                    </div>
